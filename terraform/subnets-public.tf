...
# A new AWS Subnet resource for public instances was created in order to handle new CIDR
resource "aws_subnet" "new-public-subnet" {
  vpc_id                  = aws_vpc.new_vpc.id                # ID of the VPC in which the subnet will be created
  cidr_block              = local.new_vpc.cidr_subnet_public  # CIDR block for the subnet
  map_public_ip_on_launch = true                              # Assign public IPs to instances launched in this subnet
  availability_zone       = "us-east-1b"                      # Availability zone for the subnet

  tags = {
    Name        = "${local.prefix}-new-public-subnet"         # Tag the subnet with a descriptive name
    Environment = local.env                                   # Tag the subnet with the environment
    Path        = "${basename(abspath(path.module))}/subnets-public.tf"   # Tag the subnet with the configuration file path
  }
