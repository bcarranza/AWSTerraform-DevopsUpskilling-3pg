...
resource "aws_subnet" "public-subnet-upskilling" {
  vpc_id                  = aws_vpc.upskilling_vpc.id              # ID of the VPC in which the subnet will be created
  cidr_block              =  local.upskilling_vpc_variables.upskilling_cidr_subnet_pub
  
  map_public_ip_on_launch = true                         # Assign public IPs to instances launched in this subnet
  availability_zone       = "us-east-1b"                 # Availability zone for the subnet

  tags = {
    Name        = "${local.prefix}-public-subnet-upskilling"      # Tag the subnet with a descriptive name
  }
