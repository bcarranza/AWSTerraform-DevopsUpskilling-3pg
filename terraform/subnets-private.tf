...
# Create an AWS Subnet resource for private instances in the first WEST availability zone (us-east-1b) for module 3
resource "aws_subnet" "public-subnet-west-1" {
  vpc_id                  = aws_vpc.this.id               # ID of the VPC in which the subnet will be created
  cidr_block              = local.vpc.cidr_private_subnet # CIDR block for the subnet
  ...
  tags = {
    Name        = "${local.prefix}-private-subnet-west-1"      # Tag the subnet with a descriptive name
    Environment = local.env                              # Tag the subnet with the environment
