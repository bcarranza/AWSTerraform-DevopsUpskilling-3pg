...
# Create an AWS Subnet resource for private instances in the third availability zone (us-east-1b)
resource "aws_subnet" "private-subnet-3" {
  vpc_id                  = aws_vpc.vpc2.id                # ID of the VPC in which the subnet will be created
  cidr_block              = local.vpc2.cidr_subnet_private # CIDR block for the subnet
  map_public_ip_on_launch = false                         # Do not assign public IPs to instances launched in this subnet
  availability_zone       = "us-east-1b"                  # Availability zone for the subnet
